...
class BarangMasukContent extends StatefulWidget {
  const BarangMasukContent({super.key});

  @override
  _BarangMasukContentState createState() => _BarangMasukContentState();
}

class _BarangMasukContentState extends State<BarangMasukContent> {
...
void dispose() {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      setState(() {
        stockInData = List<Map<String, dynamic>>.from(response);
        isLoading = false;
      });
    } catch (e) {
  ...
  Future<void> _addStock() async {
    if (_formKey.currentState!.validate() && selectedProductId != null) {
       _formKey.currentState!.save(); // Pastikan form disave dulu
      try {
        setState(() => isLoading = true);

        // Gunakan nilai dari controller atau dari variabel stockToAdd
      final stockValue = double.parse(_stockController.text);
        
        await supabase.from('stok_masuk').insert({
          'product_id': selectedProductId,
          'brg_masuk': stockValue,
        });
        ...
        _stockController.clear(); // Clear controller
  ...
  Future<List<Map<String, dynamic>>> _getChartData() async {
    ...
    final response = await supabase
        .from('stok_masuk')
        .select('created_at, brg_masuk, product_id, products:product_id (name)')
    ...
    return grouped.entries.map((entry) {
      ...
      final total = entry.value.fold<double>(
        0,
        (sum, record) => sum + (record['brg_masuk'] as num).toDouble(),
      );
    ...
    try {
      ...
      if (pngBytes != null) {
        ...
        final anchor = html.AnchorElement(href: url)
          ..setAttribute('download', 'grafik_stok_masuk_${DateFormat('yyyyMMdd').format(DateTime.now())}.png')
          ..click();
  ...
  Future<void> _exportAsCSV() async {
    try {
      ...
      final url = html.Url.createObjectUrlFromBlob(blob);
      final anchor = html.AnchorElement(href: url)
        ..setAttribute('download', 'data_stok_masuk_${DateFormat('yyyyMMdd').format(DateTime.now())}.csv')
        ..click();
  ...
  Future<void> _exportAsPDF() async {
    try {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (pw.Context context) => [
            pw.Header(
              level: 0,
              child: pw.Text('Laporan Stok Masuk - $now',
                  style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
            ),
            pw.SizedBox(height: 20),
            pw.Text('Grafik Stok Masuk', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
      ...
      final url = html.Url.createObjectUrlFromBlob(blob);
      final anchor = html.AnchorElement(href: url)
        ..setAttribute('download', 'laporan_stok_masuk_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf')
        ..click();
  ...
  Future<void> _downloadAllDatasets() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Dataset stok masuk berhasil diunduh')),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                 ...
                 TextFormField(
  controller: _stockController, // Tambahkan controller
  decoration: const InputDecoration(
                    ...
                    children: [
                              ...
                              PopupMenuButton<String>(
                                ...
                                itemBuilder: (BuildContext context) => [
                                  const PopupMenuItem<String>(
                                    value: 'all_data',
                                    child: Text('Unduh Dataset Stok Masuk'),
                                  ),
                                ],
                              ),
                            ],
                      ...
                      SingleChildScrollView(
                        ...
                        child: DataTable(
                          ...
                          rows: stockInData.asMap().entries.map((entry) {
                            final index = entry.key;
                            ...
                            return DataRow(
                              cells: [
                                ...
                                DataCell(Text(NumberFormat.decimalPattern('id').format(item['brg_masuk'] ?? 0))),
                                DataCell(Text(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Statistik stok masuk berdasarkan periode',
                        style: TextStyle(
                          color: Colors.grey[600],
                          fontSize: 14,
                        ),
